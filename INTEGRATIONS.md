...
**cheatsense** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `cheatsense` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
cheatsense ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
cheatsense ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
cheatsense ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `cheatsense` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="cheatsense") for rec in records]   # records = your JSON output
