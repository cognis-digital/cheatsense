"""Native cognis-connect emit for cheatsense — forward findings to any platform.

Maps cheatsense's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    cheatsense ... --format json | cheatsense-emit --to stix
    cheatsense-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop('log_line', None)
        out.pop('score', None)
        out.pop('apm', None)
        out.pop('flagged', None)
        out['title'] = rec.get('flag', 'Unknown Flag')
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'anomaly')
